...
the same workload without offload as the host-memory baseline. Each CSV row
also records aggregate storage-unit RSS before PUT, after the PUT reaches its
steady state, and after CLEAR reaches its steady state. The retained RSS column
is the CLEAR value minus the same iteration's pre-PUT baseline; the SSD case
also reports active offloaded bytes after PUT.
